...
//volatile unsigned long GreenLED = GPIO_PORTF_DATA_R;
...
void WaitForASLow(void)
{
  volatile unsigned long pf4 = GPIO_PORTF_DATA_R & 0x00000010;
  if (pf4 == 16) // SW1 not pressed
  {
    while ((GPIO_PORTF_DATA_R & 0x00000010) == 16)
    {
    }
  }
  else if (pf4 == 0) // SW1 is pressed
  {
    return;
  }
}
...
void WaitForASHigh(void)
{
  unsigned long pf4 = GPIO_PORTF_DATA_R & 0x00000010;
  if (pf4 == 16)
  {
    return;
  }
  else if ((GPIO_PORTF_DATA_R & 0x00000010) == 0)
  {
    while (!((GPIO_PORTF_DATA_R & 0x00000010) == 16))
    {
    }
...
void SetVT(void)
{
  GPIO_PORTF_DATA_R = GPIO_PORTF_DATA_R | 0x02; // sets LED to red
}
...
void ClearVT(void)
{
  GPIO_PORTF_DATA_R = GPIO_PORTF_DATA_R & 0xFFFFFFFD; // turns red LED off
}
...
void SetReady(void)
{
  GPIO_PORTF_DATA_R = GPIO_PORTF_DATA_R | 0x08; // makes green LED on
}
...
void ClearReady(void)
{
  GPIO_PORTF_DATA_R = GPIO_PORTF_DATA_R & 0xFFFFFFF7; // makes green LED off
}
...
void Delay1ms(unsigned long msec)
{
  ...
  // This loop takes 1ms to complete
  while (msec > 0)
  {
    i = 13333;
    while (i > 0)
    ...
    msec--; // msec decreases every milisecond
  }
}
